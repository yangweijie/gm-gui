...
//            $this->app->getIntegrationManager()->getMainWindow();
            $self = $this; // 保存当前实例的引用
            App::queueMain(function()use($self, $inputData, $options, $privateKey) {
                // 执行签名操作
                ...
                if ($result->success) {
                    ...
                    // 生成代码示例
                    $code = $self->app->getCodeGenerationService()->generateSm2SignCode(
                        $inputData,
                        $privateKey,
                        $options
                    );
                    ...
                    // 显示代码下载对话框
                    $self->showCodeDownloadDialog($code);
                } else {
