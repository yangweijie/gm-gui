...
class Sm2EncryptTab extends LibuiComponent
{
    /**
     * 应用实例
     *
    ...
    protected function createOptionsGroup(LibuiVBox $container): void
    {
        ...
        $container->append($optionsGroup, false); // 选项区域不扩展
    ...
    protected function createActionButtonsGroup(LibuiHBox $container): void
    {
        ...
        $container->append($this->encryptButton, false);
    }
...
//            );
            ...
            $result = $sm2Service->encrypt($inputData, $options);
            $this->app->log(var_export($result, true));
            ...
            if ($result->success) {
                ...
                var_dump(111);
                // 生成代码示例
                $code = $this->app->getCodeGenerationService()->generateSm2EncryptCode(
                    $inputData,
                    $publicKey,
                    $options
                );
    ...
    {
        // 通过父组件链查找SM2主选项卡
        // 在当前结构中，父组件是LibuiTab，需要继续向上查找直到找到Sm2MainTab
